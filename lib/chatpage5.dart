...
class ChatPage5 extends StatefulWidget {
  ...
  ChatPage5({required this.currentWallpaper, required this.updateWallpaperCallback});

  @override
  _ChatPage5State createState() => _ChatPage5State();
}
...
class _ChatPage5State extends State<ChatPage5> {
  ...
  Widget build(BuildContext context) {
    final chatviewmodel5 = Provider.of<Sportsnutrition>(context);
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('Sports & Performance Nutrition'),
        actions: <Widget>[
          IconButton(
          ...
          children: [
            Expanded(
              child: Stack(
                children: [
                  ListView.builder(
                    itemCount: chatviewmodel5.messages.length,
                    controller: _scrollController,
                    itemBuilder: (context, index) {
                      final message = chatviewmodel5.messages[index];
                      return Align(
                        alignment:
                  ...
                  if (chatviewmodel5.messages.isEmpty)
                    Center(
                      child: Padding(
                        padding: const EdgeInsets.symmetric(horizontal: 24.0),
                        child: Text(
                          'Ask me anything about     Sports & Performance Nutrition',
                    ...
                    child: IconButton(
                      ...
                      onPressed: () {
                        if (_controller.text.trim().isNotEmpty) {
                          chatviewmodel5.sendMessage(_controller.text.trim());
                          _controller.clear();
                          _scrollToBottom();
                        }
                      },
